...
class PersonalizedDailyCardService {
  ...
  async sendPersonalizedEmail(email: string, cardName: string, teaser: string, date: string, userId: string) {
    try {
      // Template de email simple y ambiguo para generar curiosidad
      const emailContent = this.generateSimpleEmailTemplate(email, cardName, teaser, date, userId);
  ...
  generateSimpleEmailTemplate(email: string, cardName: string, teaser: string, date: string, userId: string): string {
    ...
    <div style="max-width: 480px; margin: 0 auto; background: linear-gradient(135deg, #16213e 0%, #0f3460 100%);">
        ...
        <div style="padding: 30px 25px; text-align: center; color: white;">
            ...
            <div style="margin: 30px 0 20px;">
                ...
                <a href="${process.env.NEXT_PUBLIC_SITE_URL || 'https://videntia.com'}/api/track/mail-click?uid=${encodeURIComponent(userId)}&card=${encodeURIComponent(cardName)}&date=${encodeURIComponent(date)}" 
    ...
    <div style="max-width: 500px; margin: 0 auto; background: #f7fafc; border-radius: 12px; overflow: hidden; box-shadow: 0 10px 25px rgba(0, 0, 0, 0.2);">
        ...
        <div style="padding: 30px 25px; text-align: center;">
            ...
            <div style="margin: 25px 0;">
                <a href="${process.env.NEXT_PUBLIC_SITE_URL || 'https://videntia.com'}/api/track/mail-click?uid=${encodeURIComponent(reading.user_id)}&card=${encodeURIComponent(reading.card_name)}&date=${encodeURIComponent(reading.date_for)}" 
